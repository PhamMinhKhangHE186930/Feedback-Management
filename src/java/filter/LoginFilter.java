...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        boolean isLosinPage = uri.contains("login.jsp") || uri.contains("login");

        // check if the session of account and if user access to the login page is already exist or not
        if (session.getAttribute("account") == null && !isLosinPage && !uri.contains("createaccount")) {
            // check if there are cookies or not
            if (arrCookies != null) {
                // if there a has cookies
                ...
                // create object account to send the session
                ...
                if (account != null) {
                    ...
                    if(account.getRole() == 1){
                        res.sendRedirect("supporthome");
                    }else if(account.getRole() == 2){
                        res.sendRedirect("studenthome");
                    }
                } else {
                ...
                // if user do not choose remember me
                req.setAttribute("message", "You need to login first");
                req.getRequestDispatcher("login.jsp").forward(request, response);
                return;
            }
